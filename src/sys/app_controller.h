...
#define APP_CONTROLLER_NAME 16   // app控制器的名字长度
...
{
public:
    AppController(const char *name = "AppCtrl");
    ~AppController();
    ...
    bool ble_event(APP_EVENT_TYPE type);

private:
    int app_is_legal(const APP_OBJ *app_obj);

private:
    char name[APP_CONTROLLER_NAME]; // app控制器的名字
    ...
    boolean m_ble_status;

    unsigned long m_preWifiReqMillis; // 保存上一回请求的时间戳
    unsigned long m_preBLEReqMillis;
